...
<section class="bo-crud-head">
    <div>
        ...
        <h1><?= esc(lang('Backoffice.ntf_complainant_details_title')) ?></h1>
        <p><?= esc(lang('Backoffice.ntf_details_lead')) ?></p>
    </div>
    <div class="bo-crud-head-actions">
        <?php if (can_access('backoffice/notifications/complainants/resend')): ?>
        <form method="post" action="<?= site_url('backoffice/notifications/complainants/' . $record['id'] . '/resend') ?>" onsubmit="return confirm('<?= esc(lang('Backoffice.ntf_resend_confirm'), 'js') ?>');">
            <?= csrf_field() ?>
        ...
        <?php endif; ?>
        <a class="btn btn-bo-secondary" href="<?= site_url('backoffice/notifications/complainants') ?>">
...
<section class="bo-panel bo-crud-panel">
    <h2 class="bo-form-section-title"><?= esc(lang('Backoffice.ntf_section_recipient')) ?></h2>
    <dl class="bo-detail-list">
        <div><dt><?= esc(lang('Backoffice.ntf_col_recipient')) ?></dt><dd><?= esc($record['recipient']) ?></dd></div>
        <div><dt><?= esc(lang('Backoffice.users_col_cni')) ?></dt><dd><?= esc($record['recipient_cni'] ?: '—') ?></dd></div>
    ...
    <h2 class="bo-form-section-title mt-4"><?= esc(lang('Backoffice.ntf_section_complaint')) ?></h2>
    <dl class="bo-detail-list">
        <div><dt><?= esc(lang('Backoffice.ntf_col_case')) ?></dt><dd><?= esc($record['case_number']) ?></dd></div>
        <div><dt><?= esc(lang('Backoffice.ntf_field_case_subject')) ?></dt><dd><?= esc($record['case_subject']) ?></dd></div>
    </dl>
