...
$action = $isEdit
    ? site_url('backoffice/appeals/' . (int) ($record['recours_id'] ?? 0))
    : site_url('backoffice/appeals');
...
$deadlineDate = $record['date_limite_recours'] ?? '';
$within       = $record['dans_les_delais'] ?? null;
$withinLabel  = $within === null || $within === ''
    ? '—'
    : (filter_var($within, FILTER_VALIDATE_BOOLEAN) ? lang('Backoffice.yes') : lang('Backoffice.no'));
?>

<section class="bo-crud-head">
    <div>
        <p class="bo-crud-kicker"><?= esc(lang('Backoffice.nav_appeals')) ?></p>
        <h1><?= esc($isEdit ? lang('Backoffice.apl_edit_title') : lang('Backoffice.apl_create_title')) ?></h1>
        <p><?= esc(lang('Backoffice.apl_form_lead')) ?></p>
    </div>
...
<section class="bo-panel bo-crud-panel">
    <form class="bo-form needs-validation" method="post" action="<?= esc($action) ?>" enctype="multipart/form-data" novalidate
          ...
          data-api-doc-types="<?= esc(site_url('backoffice/api/appeal-document-types'), 'attr') ?>"
          data-api-parent="<?= esc(site_url('backoffice/api/appeal-parents'), 'attr') ?>"
          data-parents-json="<?= esc(json_encode($parents, JSON_UNESCAPED_UNICODE), 'attr') ?>"
          data-empty-docs="<?= esc(lang('Backoffice.apl_docs_hint'), 'attr') ?>">
        <?= csrf_field() ?>

        <h2 class="h5"><?= esc(lang('Backoffice.apl_section_parent')) ?></h2>
        <div class="row g-3 mb-3">
            <div class="col-12">
                <label class="form-label" for="plainte_parent_id"><?= esc(lang('Backoffice.apl_field_parent')) ?> *</label>
                <select class="form-select" id="plainte_parent_id" name="plainte_parent_id" data-apl="parent" required>
                    <option value=""><?= esc(lang('Backoffice.select_placeholder')) ?></option>
                    <?php foreach ($parents as $opt): ?>
                        <option value="<?= esc($opt['id']) ?>"
                                data-next-niveau="<?= esc($opt['next_niveau_id'] ?? '') ?>"
                                data-deadline="<?= esc($opt['date_limite_recours'] ?? '') ?>"
                                <?= (string) $val($record, 'plainte_parent_id') === (string) $opt['id'] ? 'selected' : '' ?>><?= esc($opt['label']) ?></option>
                    <?php endforeach; ?>
                </select>
                <div class="invalid-feedback"><?= esc(lang('Backoffice.validation_required')) ?></div>
            </div>
            <div class="col-12">
                <div id="apl-deadline-info" class="alert alert-secondary mb-0" role="status">
                    <strong><?= esc(lang('Backoffice.apl_field_deadline')) ?>:</strong>
                    <span data-apl-deadline-date><?= esc($deadlineDate !== '' ? $deadlineDate : '—') ?></span>
                    — <?= esc(lang('Backoffice.apl_field_within_deadline')) ?>:
                    <span data-apl-deadline-within><?= esc($withinLabel) ?></span>
                </div>
            </div>
        </div>

        <h2 class="h5"><?= esc(lang('Backoffice.apl_section_general')) ?></h2>
        <div class="row g-3 mb-4">
            <div class="col-12">
                <label class="form-label" for="objet"><?= esc(lang('Backoffice.apl_field_objet')) ?> *</label>
                <input class="form-control" id="objet" name="objet" value="<?= esc($val($record, 'objet')) ?>" required>
                <div class="invalid-feedback"><?= esc(lang('Backoffice.validation_required')) ?></div>
            </div>
            <div class="col-12">
                <label class="form-label" for="description"><?= esc(lang('Backoffice.apl_field_description')) ?> *</label>
                <textarea class="form-control" id="description" name="description" rows="4" required><?= esc($val($record, 'description')) ?></textarea>
                <div class="invalid-feedback"><?= esc(lang('Backoffice.validation_required')) ?></div>
            </div>
            <div class="col-md-3">
                <label class="form-label" for="niveau_juridiction_id"><?= esc(lang('Backoffice.apl_field_level')) ?> *</label>
                <select class="form-select" id="niveau_juridiction_id" name="niveau_juridiction_id" data-apl="niveau" required>
                    <option value=""><?= esc(lang('Backoffice.select_placeholder')) ?></option>
            ...
            <div class="col-md-3">
                <label class="form-label" for="province_id"><?= esc(lang('Backoffice.apl_field_province')) ?> *</label>
                <select class="form-select" id="province_id" name="province_id" data-apl="province" required>
                    <option value=""><?= esc(lang('Backoffice.select_placeholder')) ?></option>
            ...
            <div class="col-md-3">
                <label class="form-label" for="commune_id"><?= esc(lang('Backoffice.apl_field_commune')) ?> *</label>
                <select class="form-select" id="commune_id" name="commune_id" data-apl="commune" required>
                    <option value=""><?= esc(lang('Backoffice.select_placeholder')) ?></option>
            ...
            <div class="col-md-3">
                <label class="form-label" for="juridiction_id"><?= esc(lang('Backoffice.apl_field_court')) ?> *</label>
                <select class="form-select" id="juridiction_id" name="juridiction_id" data-apl="juridiction" required>
                    <option value=""><?= esc(lang('Backoffice.select_placeholder')) ?></option>
        ...
        <h2 class="h5"><?= esc(lang('Backoffice.apl_section_parcels')) ?></h2>
        <div id="apl-parcels" data-parcel-rows>
        ...
        <h2 class="h5"><?= esc(lang('Backoffice.apl_section_parties')) ?></h2>
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                <label class="form-label" for="complainant_ids"><?= esc(lang('Backoffice.apl_field_complainants')) ?> *</label>
            ...
            <div class="col-md-4">
                <label class="form-label" for="defendant_ids"><?= esc(lang('Backoffice.apl_field_defendants')) ?> *</label>
            ...
            <?php if ($hasWitness): ?>
            <div class="col-md-4">
                <label class="form-label" for="witness_ids"><?= esc(lang('Backoffice.apl_field_witnesses')) ?></label>
        ...
        <h2 class="h5"><?= esc(lang('Backoffice.apl_section_documents')) ?></h2>
        <div id="apl-documents" class="mb-3" data-doc-types>
            ...
            <?php endforeach; ?>
            <?php if ($docTypes === []): ?>
                <p class="text-muted"><?= esc(lang('Backoffice.apl_docs_hint')) ?></p>
            <?php endif; ?>
        </div>

        <div class="bo-form-actions">
            <button class="btn btn-bo-primary" type="submit"><i class="bi bi-check-lg"></i> <?= esc($isEdit ? lang('Backoffice.apl_save') : lang('Backoffice.apl_create')) ?></button>
            <a class="btn btn-bo-secondary" href="<?= site_url('backoffice/appeals') ?>"><?= esc(lang('Backoffice.btn_cancel')) ?></a>
        </div>
    </form>
</section>
<?= $this->endSection() ?>
