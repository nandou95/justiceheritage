        ...
        <table class="table table-hover bo-table jh-datatable w-100" id="csc-table" data-page-length="10" data-order-col="0" data-order-dir="asc" data-dom="lrtip">
            <thead>
                <tr>
                    ...
                    <th><?= esc(lang('Backoffice.csc_col_profiles_current')) ?></th>
                    <th><?= esc(lang('Backoffice.csc_col_stage_next')) ?></th>
                    <th><?= esc(lang('Backoffice.csc_col_level_next')) ?></th>
                    <th><?= esc(lang('Backoffice.csc_col_profiles_next')) ?></th>
            ...
            <?php foreach ($items as $row): ?>
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn btn-link p-0 bo-count-link" data-bo-csc-profiles
                            data-etape-id="<?= (int) $row['etape_actuel_id'] ?>"
                            data-name="<?= esc($row['etape_actuel'], 'attr') ?>">
                            <?= (int) $row['profiles_actuel_count'] ?>
                        </button>
                    </td>
                    ...
                    <td>
                        <button type="button" class="btn btn-link p-0 bo-count-link" data-bo-csc-profiles
                            data-etape-id="<?= (int) $row['etape_suivant_id'] ?>"
                            data-name="<?= esc($row['etape_suivant'], 'attr') ?>">
                            <?= (int) $row['profiles_suivant_count'] ?>
                        </button>
                    </td>
            ...
            <div class="modal-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="niveau_juridiction_actuel_id" name="niveau_juridiction_actuel_id" data-csc="niveau-actuel" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="etape_plainte_actuel_id" name="etape_plainte_actuel_id" data-csc="etape-actuel" required>
                            <option value=""><?= esc(lang('Backoffice.select_placeholder')) ?></option>
                        </select>
                        <div class="invalid-feedback"><?= esc(lang('Backoffice.validation_required')) ?></div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="niveau_juridiction_suivant_id" name="niveau_juridiction_suivant_id" data-csc="niveau-suivant" required>
                            <option value=""><?= esc(lang('Backoffice.select_placeholder')) ?></option>
                            <?php foreach ($levels as $opt): ?>
                                <option value="<?= esc($opt['id']) ?>"><?= esc($opt['label']) ?></option>
                            <?php endforeach; ?>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="etape_plainte_suivant_id" name="etape_plainte_suivant_id" data-csc="etape-suivant" required>
                            <option value=""><?= esc(lang('Backoffice.select_placeholder')) ?></option>
                        </select>
                        <div class="invalid-feedback"><?= esc(lang('Backoffice.validation_required')) ?></div>
                    </div>
...
<div class="modal fade" id="cscProfilesModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable"><div class="modal-content">
        <div class="modal-header">
            <h2 class="modal-title fs-5" id="cscProfilesModalTitle"><?= esc(lang('Backoffice.cs_profiles_title')) ?></h2>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
            <div class="table-responsive">
                <table class="table table-sm bo-table" id="cscProfilesTable">
                    <thead>
                        <tr>
            ...
            <p class="text-muted mb-0 d-none" id="cscProfilesEmpty"><?= esc(lang('Backoffice.cs_profiles_empty')) ?></p>
        </div>
    </div></div>
</div>
...
window.JH_CSC_I18N = {
    ...
    profilesUrl: <?= json_encode(site_url('backoffice/complaint-stage-configs/profiles/__ID__')) ?>,
