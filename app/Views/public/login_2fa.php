...
<section class="jh-auth" aria-label="<?= esc(lang('Site.login_2fa_h1')) ?>">
    ...
    <div class="jh-auth-panel">
        <div class="jh-auth-panel-inner">
            ...
            <h1><?= esc(lang('Site.login_2fa_h1')) ?></h1>
            <p class="jh-auth-lead">
                <?= esc(lang('Site.login_2fa_lead', [$emailMasked ?? '***'])) ?>
            </p>

            <ol class="jh-auth-steps" aria-label="Authentication steps">
                <li class="is-done"><?= esc(lang('Site.login_step_password')) ?></li>
                <li class="is-active"><?= esc(lang('Site.login_step_2fa')) ?></li>
            </ol>
            ...
            <form class="jh-auth-form" method="post" action="<?= site_url('login/2fa') ?>" novalidate autocomplete="one-time-code">
                <?= csrf_field() ?>
                <div class="jh-field">
                    <label class="form-label" for="code"><?= esc(lang('Site.login_2fa_code')) ?></label>
                    <input class="form-control" type="text" inputmode="numeric" pattern="[0-9]{6}" maxlength="6"
                           id="code" name="code" required autofocus
                           autocomplete="one-time-code">
                </div>
                <button class="btn btn-jh-primary jh-auth-submit" type="submit"><?= esc(lang('Site.login_2fa_submit')) ?></button>
            </form>

            <form method="post" action="<?= site_url('login/2fa/resend') ?>" class="mt-3">
                <?= csrf_field() ?>
                <button class="btn btn-jh-secondary w-100" type="submit"><?= esc(lang('Site.login_2fa_resend')) ?></button>
            </form>

            <p class="jh-auth-footer mt-3">
                <a href="<?= site_url('login') ?>"><?= esc(lang('Site.login_2fa_back')) ?></a>
            </p>
        </div>
    </div>
</section>
